...
- [InputValidator] 입력값이 올바른 구분자 형태인지(쉼표(,)로 구분되어 있는지) 확인한다.
    - 입력값이 비어있거나 공백뿐인 경우 예외 발생
    - 입력값에 한글, 영문, 쉼표(,), 공백 이외의 문자가 포함된 경우 예외 발생
    - 입력값이 쉼표(,)로 구분되어 있더라도, 쉼표 사이 빈 항목이 있으면 `InputValidator`는 빈 항목을 제거(정제)
        - 예: "pobi,,woni" ➔ "pobi,woni"
    - 정제 결과(빈 항목 제거 후)에 유효한 이름이 하나도 남아있지 않는 경우 예외 발생
- [RacingCarParser] 정제된 문자열을 쉼표(,) 기준으로 자동차 이름을 분리한다.
- [RacingCarValidator]
...
- [입력 형식 오류] 입력값이 비어있거나 공백뿐인 경우 ➔ `[ERROR] 자동차 이름을 입력해 주세요.`
- [입력 형식 오류] 입력값에 허용되지 않은 문자가 포함된 경우 ➔ `[ERROR] 자동차 이름은 한글과 영문, 쉼표(,)만 입력 가능합니다.`
- [입력 형식 오류] 정제 결과(빈 항목 제거 후)에 유효한 이름이 하나도 남아있지 않는 경우 ➔ `[ERROR] 유효한 자동차 이름이 없습니다.`
- [자동차 검증] 자동차 이름이 하나 이하인 경우 ➔ `[ERROR] 경주에 참여할 자동차는 최소 2대 이상이어야 합니다.`
- [자동차 검증] 자동차 수가 10대를 초과하는 경우 ➔ `[ERROR] 최대 10대까지만 출전 가능합니다.`
- [자동차 검증] 이름이 1자 이상 5자 이하가 아닌 경우 ➔ `[ERROR] 자동차 이름은 1자 이상 5자 이하만 가능합니다.`
- [자동차 검증] 자동차 이름이 중복되어 입력될 경우 ➔ `[ERROR] 자동차 이름이 중복되었습니다.`
...
- 사용자 입력값에 쉼표가 포함되어도 쉼표 사이 빈 항목이 있으면 빈 항목을 제거(정제) 후 프로그램이 진행되도록 한다.  
  즉 사용자가 쉼표를 잘못 입력했더라도, 사용자에게 편리하도록 작은 실수는 허용하여 자동차 게임 진행 가능하도록 하였다.
    - 예: `pobi,,woni` ➔ `pobi,woni`
    - 예: `,,` ➔ `[ERROR] 유효한 자동차 이름이 없습니다.`
...
![최종 자동차 경주 게임 흐름](./assets/final_racing_car_game.jpg)
