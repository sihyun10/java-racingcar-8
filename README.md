...
- 자동차 객체가 스스로 전진 조건을 판단하고 이동
- `RandomNumberGenerator`를 통해 생성된 0에서 9 사이의 숫자가 4 이상일 때 전진

### 2. 자동차 집합의 관리 (Cars)

여러 자동차에게 전진 명령을 내리고, 그들의 상태를 종합해 "우승자"를 판단 ➔ 경주의 진행과 결과 계산을 담당
...
여러 명의 우승자가 존재할 수 있으므로, 최대 거리와 동일한 distance를 가진 자동차 목록을 관리
